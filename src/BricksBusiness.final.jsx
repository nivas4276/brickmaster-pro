...
function AuthScreen({ onAuth }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "signup") {
        setMsg("Account created. If email confirmation is enabled, verify your email and then log in.");
        return;
      }
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background: `linear-gradient(135deg, ${T.bg} 0%, #fff7ed 100%)`,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        padding: 18,
    ...
    >
      <div
        style={{
          ...
          width: "100%",
          maxWidth: 430,
          boxShadow: "0 18px 50px rgba(31, 41, 51, 0.12)",
        }}
      >
        <div
          style={{
        ...
        <h2 style={{ margin: 0, color: T.text }}>BrickMaster Pro</h2>
        <p style={{ color: T.muted, marginTop: 6, lineHeight: 1.45 }}>
          Secure access for your bricks business records, customers, dues and payments.
        </p>

        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 18 }}>
          <Inp label="Email" type="email" value={email} onChange={setEmail} />
          <Inp label="Password" type="password" value={password} onChange={setPassword} />

          {msg && (
            <div style={{ color: mode === "signup" ? T.green : T.red, fontSize: 13, lineHeight: 1.4 }}>
              {msg}
            </div>
          )}

          <Btn onClick={submit} color={T.accent}>
...
export default function App() {
  ...
  const { data, loading, error, reload } = useData(Boolean(session));
